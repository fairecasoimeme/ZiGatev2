...
extern const iocon_group_t dk6_spifi_io[6];
void BOARD_InitSPIFI( void )
{
    uint32_t divisor;
    int nb_spifi_pins = sizeof(dk6_spifi_io)/sizeof(iocon_group_t);
    IOCON_SetPinMuxing(IOCON, dk6_spifi_io, nb_spifi_pins);
...
{
    int nb_buttons = sizeof(dk6_button_io_pins)/sizeof(gpioInputPinConfig_t);
    uint32_t u32SkipIO = 0;

    for (int i = 0; i < nb_buttons; i++)
    {
        u32SkipIO |= (1 << dk6_button_io_pins[i].gpioPin);
    }

    for (int i=0; i < GPIO_B_COUNT2; i++)
    {
        if (((u32SkipIO >> i) & 0x1) != 1)
        {
            /* configure GPIOs to Input mode */
            GPIO_PinInit(GPIO, 0, i, &((const gpio_pin_config_t){kGPIO_DigitalInput, 1}));
            IOCON_PinMuxSet(IOCON, 0, i, IOCON_MODE_INACT | IOCON_FUNC0 | IOCON_ANALOG_EN);
        }
    }
}
