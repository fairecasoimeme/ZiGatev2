...
static void APP_SendLedColorWheel(void);
...
{
    otCliOutputFormat("Welcome to JN5189 OpenThread REED demo, default channel is %d.\r\nDone\r\n", gChannel);
...
static void APP_OtNwkJoin()
{
    ...
    if ((isOutOfBand == FALSE) && (FALSE == isCommissioned))
    {
...
)
{
    switch(keyEvent)
    {
        ...
        case gKBD_EventLongPB1_c:
            ...
            break;
        case gKBD_EventLongPB2_c:
            otCliOutputFormat("Led Colorwheel\r\n");
            APP_SendLedColorWheel();
            break;
...
{
#if gLEDSupported_d
    APP_SetState(deviceState);
    Led_SetState(APP_GetMode(), APP_GetState());
#endif
}
...
static void APP_GetBoardTemperatureString(uint8_t *pString)
{
    ...
    BOARD_GetTemperature(&tempInt, &tempDec);
    pIndex = pString;
    ...
    *pIndex = '.';
    pIndex++;
    NWKU_PrintDec(tempDec, pIndex, 2, TRUE);
}
#if gKBD_KeysCount_c > 2
/*!*************************************************************************************************
\private
