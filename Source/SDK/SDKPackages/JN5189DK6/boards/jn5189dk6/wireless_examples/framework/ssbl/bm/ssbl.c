...
)
{
    ...
    for (int i = 0; i < IMG_DIRECTORY_MAX_SIZE; i++)
    {
        ...
        if (dir_entry_iterator->img_type == imgType)
        {
...
static StatusOta_t ssbl_GetOtaFile(const image_directory_entry_t * ota_entry,
                              ...
                              const IMAGE_CERT_T * root_cert, uint32_t end_of_ssbl)
{
...
//#define WAIT_DEBUG
    ...
    do {
        ...
        {
            ssbl_GetOtaFile(&page[0].page0_v3.ota_entry, &page[0].page0_v3.img_directory[0], &page[0].page0_v3.zigbee_password[0], root_cert, end_of_ssbl);
